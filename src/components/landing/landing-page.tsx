  ...
  return (
    <main className="min-h-screen p-4 md:p-8 overflow-x-hidden">
      ...
      <div className="max-w-4xl mx-auto notebook-page min-h-[90vh] p-8 md:p-16 rounded-sm relative transform rotate-[0.5deg]">
        ...
        {/* Header / Nav */}
        <nav className="flex justify-between items-center mb-12 relative z-20 pl-2 md:pl-8">
          <div className="text-2xl md:text-3xl font-bold text-slate-800 tracking-tighter transform -rotate-2">
            Diary Maker ✏️
          </div>
          <Link 
            href="/login"
            className="bg-slate-800 text-white px-4 md:px-6 py-2 rounded-full transform hover:scale-105 transition-all shadow-lg hover:shadow-xl font-sans text-xs md:text-sm tracking-wide"
          >
            로그인하기
          </Link>
        </nav>
        ...
        {/* Hero Section */}
        <div className="pl-2 md:pl-8 relative z-20">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              <div className="inline-block px-3 py-1 bg-yellow-100 text-yellow-800 rounded-lg text-base md:text-lg transform -rotate-1 shadow-sm border border-yellow-200">
                ✨ 오늘도 수고한 당신에게
              </div>
              <h1 className="text-4xl md:text-5xl lg:text-6xl text-slate-900 leading-tight handwritten">
                오늘의 짧은 생각,<br/>
                <span className="marker-highlight">따뜻한 일기</span>가 되다.
              </h1>
              <p className="text-lg md:text-xl text-slate-600 leading-relaxed pt-4">
                &quot;야근, 비 옴, 편의점...&quot;<br/>
                단어 몇 개만 던져주세요. <br/>
              ...
              <div className="pt-8 flex flex-wrap gap-4">
                <Link 
                  href="/login"
                  className="bg-indigo-600 text-white text-lg md:text-xl px-6 md:px-8 py-3 md:py-4 rounded-xl shadow-md hover:bg-indigo-700 transition-colors flex items-center gap-2 font-sans"
                >
                  일기 쓰러 가기 📒
                </Link>
                <button className="px-6 md:px-8 py-3 md:py-4 text-indigo-700 underline underline-offset-4 decoration-wavy decoration-indigo-300 hover:text-indigo-900 transition-colors text-lg md:text-xl">
                  예시 보기
                </button>
              </div>
            </div>
            ...
            <div className="relative mt-8 md:mt-0 h-[350px] md:h-[400px]">
              {/* Polaroid 1 */}
              <div className="absolute top-0 right-0 md:right-4 w-56 md:w-64 bg-white p-3 pb-8 shadow-xl transform rotate-6 border border-gray-100 z-10 transition-transform hover:scale-105 hover:rotate-2 hover:z-50 duration-300 cursor-pointer">
                <div className="h-40 md:h-48 bg-slate-200 rounded-sm mb-3 overflow-hidden">
                ...
                <p className="text-lg md:text-xl text-center text-gray-700 handwritten">비 오는 날의 라면 🍜</p>
                {/* Tape element */}
                <div className="tape absolute -top-3 left-1/2 -translate-x-1/2 w-20 md:w-24 h-6 md:h-8 rotate-3"></div>
              </div>

              {/* Polaroid 2 */}
              <div className="absolute top-12 md:top-8 right-8 md:right-16 w-56 md:w-64 bg-white p-3 pb-8 shadow-xl transform -rotate-3 border border-gray-100 z-20 transition-transform hover:scale-105 hover:rotate-0 hover:z-50 duration-300 cursor-pointer">
                <div className="text-xs md:text-sm p-3 md:p-4 h-40 md:h-48 bg-gray-50 text-gray-600 overflow-hidden leading-relaxed font-sans border border-gray-100">
                  &quot;오늘은 정말 정신없는 하루였다. 갑자기 쏟아진 비에 우산도 없이 편의점으로 뛰어가 컵라면을 샀다. 후두둑 떨어지는 빗소리를 들으며 먹는 따뜻한 국물... 왠지 모르게 위로받는 기분이었다.&quot;
                </div>
                <p className="text-lg md:text-xl text-center text-gray-700 mt-2 handwritten">2026.01.16</p>
                <div className="tape absolute -top-3 left-1/2 -translate-x-1/2 w-20 md:w-24 h-6 md:h-8 -rotate-2"></div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-24 md:mt-32 space-y-16 md:space-y-24">
            ...
            {/* Feature 3 */}
            <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-center pb-12 md:pb-20">
              <div className="w-full md:w-1/2 text-center bg-gray-800 p-2 pb-6 md:pb-8 shadow-2xl relative rotate-1">
                <div className="bg-white h-40 md:h-48 flex items-center justify-center text-gray-400">
                  <span className="text-3xl md:text-4xl text-gray-300">📸 Image</span>
                </div>
          ...
          <div className="border-t-2 border-dashed border-gray-300 pt-6 md:pt-8 text-center text-slate-500 font-sans text-xs md:text-sm pb-6 md:pb-8">
